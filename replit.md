...
**Self-serve sign-up (Phase 2):** Public `/signup` page (`client/src/pages/Signup.tsx`) lets a property owner request access. Backend `POST /api/signup` provisions a `pending` property + owner admin user atomically (`storage.createPropertyWithOwner` wraps both in a `db.transaction`); defaults seeded best-effort after. Slug availability checked live via `GET /api/signup/slug-available` (guards reserved slugs in `RESERVED_SLUGS`). Super-admins are emailed on each new sign-up. Super-admin reviews from the dashboard; `POST /api/super-admin/properties/:id/approve` flips status to `active` (only when currently `pending`) and emails the owner; `POST /api/super-admin/properties/:id/reject` deletes the pending property (only when `pending`). Pending properties' admin users CAN log in (so they can prepare) but `requirePropertyUser` blocks data access until status is `active`. Tenant resolution is **fail-closed** for explicit selectors: an explicit `?property=<slug>` / `X-Property-Slug` / subdomain that doesn't resolve to an active property returns 404 (no silent fallback to default). Fallback to default property only happens when there is no tenant hint at all. Follow-ups: rate-limit `/api/signup` and `/api/signup/slug-available` (currently unrate-limited).
